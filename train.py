...
def train_net(net, args, ema_net=None):
    ...
    writer = SummaryWriter(f"{args.log_path}{args.unique_name}")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch+1) % args.val_frequency == 0:
            ...
            if dice_list_test.mean() >= best_Dice.mean():
                ...
                torch.save(net_for_eval.state_dict(), f"{args.cp_path}{args.dataset}/{args.unique_name}_best.pth")
...
def get_parser():
    ...
    parser.add_argument('--unique_name', type=str, default='ixi_resunet', help='unique experiment name') ##############
...
if __name__ == '__main__':
    ...
    configure_logger(0, args.cp_dir+f".txt")
    save_configure(args)
    logging.info(
    ...
    best_Dice, best_HD, best_ASD = train_net(net, args, ema_net)
