...
def get_model(args, pretrain=False):
    ...
    if args.dimension == '2d':
        ...
        if args.model == 'vnet':
            ...
            from .dim3 import UNet_PE
            if pretrain:
                raise ValueError('No pretrain model available')
            return UNet_PE(args.in_chan, args.base_chan, num_classes=args.classes, scale=args.down_scale, norm=args.norm, kernel_size=args.kernel_size, block=args.block)
        elif args.model == 'unet_ve':
            from .dim3 import UNet_VE
            if pretrain:
                raise ValueError('No pretrain model available')
            return UNet_VE(args.in_chan, args.base_chan, num_classes=args.classes, scale=args.down_scale, norm=args.norm, kernel_size=args.kernel_size, block=args.block)
